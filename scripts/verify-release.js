...
async function request(url){let last;for(let i=0;i<3;i++){try{const r=await fetch(url,{signal:AbortSignal.timeout(20000)});if(r.status<500)return r;last=new Error(url+' '+r.status);}catch(e){last=e;}}throw last;}
(async()=>{
 ...
 const assets=['js/catalog.js','js/cart.js','js/cart-en.js','js/currency.js','js/site-refresh.js','favicon.ico','images/favicon-192.png','images/favicon-512.png','images/paratrike-01.webp','images/paratrike-02.webp','images/shakira-real.webp','images/castillo-real.webp','images/plancton-referencia.webp','site.webmanifest'];
 for(const file of assets){const r=await request(base+'/'+file);assert.equal(r.status,200,file);assert(Buffer.from(await r.arrayBuffer()).equals(fs.readFileSync(file)),'Asset differs: '+file);}
